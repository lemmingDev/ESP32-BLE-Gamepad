...
scan-response name as *the* device name, so the alias must read as the same
device, not a separate serial gadget. The GAP/pairing names above are
untouched — Windows pairing display and the Xbox driver path don't change.
