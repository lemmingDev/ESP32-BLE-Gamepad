...
void handleCommand(String cmd)
{
    ...
    if (cmd.startsWith("trigger "))
    {
        ...
        if (s1 > 0)
        {
            ...
            if (which == "left") { bleGamepad.setLeftTrigger(v); stLT = v; NuSerial.println("ok " + cmd); }
            else if (which == "right") { bleGamepad.setRightTrigger(v); stRT = v; NuSerial.println("ok " + cmd); }
