# Testing SInputFullGamepad.ino with SDL3 on Linux

This walks through verifying SInputFullGamepad.ino end-to-end against a real
SDL3 app on Linux: buttons/axes, Player LED, and the battery ramp added in
this example G현 using [sdl3_gamepad_test.c](sdl3_gamepad_test.c), a small
...
unstable alike) is still on the 3.2.x series G현 no SInput driver. You need
...
If your distro happens to already carry G栗3.4.x (check
...
gone stale G현 `git ls-remote --tags https://github.com/libsdl-org/SDL.git` to
check)
...
[LinuxHIDTesting.md](../../LinuxHIDTesting.md) steps 1-5 unchanged G현 SInput
...
32 buttons, live. That's not incidental G현 `BleGamepad.cpp`'s SInput branch
...
output-command reports) stay opaque G현 nothing outside SDL's SInput driver
...
- `bleGamepadConfig.setEnableSInput(true)` switches the advertised VID/PID to
  ...
  pair G현 see `BleGamepadConfiguration.h`). If you'd previously paired this
  same board while it was running different firmware (a different VID/PID,
  or a different example), remove the old bonding first G현
  `bluetoothctl remove <address>` G현 then re-pair.
- If that re-pair fails with `org.bluez.Error.AuthenticationFailed` (check
  with `sudo btmon -i hci0` in a second terminal G현 you'll see
  ...
  or `pio run -t erase` under PlatformIO) rather than just re-uploading G현
...
No display needed G현 this only calls `SDL_Init(SDL_INIT_GAMEPAD)`, so it runs
...
`src/joystick/hidapi/SDL_hidapi_sinput.c` and rebuild SDL) G현 but it does
...
entirely G현 useful for isolating a firmware bug from an SDL/driver-layer one.
...
- **VID/PID** should read `0x2E8A`/`0x10C6` G현 if it shows this library's
...
- **Underlying device path** is the `hidraw` node SDL opened for this
  gamepad G현 hand it straight to `sinput_hid_test.py --device <path>` (see
...
- **Player LED capable (SDL's view)** is `SDL_PROP_GAMEPAD_CAP_PLAYER_LED_BOOLEAN`
  G현 this is the public-API window onto the internal `player_leds_supported`
...
- Every 3 seconds the test program itself calls
  `SDL_SetGamepadPlayerIndex()`, cycling 0G樣1G樣2G樣3G樣0..., and prints
  `succeeded`/`FAILED` for each call G현 watch the ESP32's own Serial Monitor
  at the same time for a matching `Player LED index: N` line. If it doesn't
  show up, see Troubleshooting below.
- **Rumble capable** should read `false` G현 this library's SInput support
...
it either (see below)**
- Re-check step 1 G현 `pkg-config --modversion sdl3` printing `3.2.x` here is
...
- To check whether hidapi sees the device at all, independent of SDL's
  ...
  device hidapi can see, with vendor/product IDs G현 a much smaller surface to
...
- This is likely a permissions issue on `/dev/hidraw*` itself, not an
  SDL/hidapi-specific one G현 see the `hidraw` udev rule in
...
- `setEnableSInput(true)` wasn't the last VID/PID-affecting call before
  `begin()` G현 see the note in step 3 above.
...
- Run [sinput_hid_test.py](sinput_hid_test.py) against the same connection
  ...
  no longer match SDL's actual `src/joystick/hidapi/SDL_hidapi_sinput.c` G현
